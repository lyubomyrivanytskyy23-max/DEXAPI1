...
def obfuscate_lua(source: str, publish=True, level="hard", minimum_size=False, target_bytes: int = 0) -> str:
    ...
    if current_code_bytes < needed_code:
        # Insert extra environment captures right after the function open so they
        # sit naturally alongside the real decoder locals and look intentional.
        # Names are generated by _junk_fenv_name() which combines meaningful Lua
        # runtime vocabulary fragments (e.g. _vmCtx, _rtScope, _gcFrame) and are
        # all registered in `used` so they never collide with real variable names.
